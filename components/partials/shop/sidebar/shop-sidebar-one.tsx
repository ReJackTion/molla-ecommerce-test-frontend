...
function ShopSidebarOne(props): JSX.Element {
  ...
  useEffect(() => {
    if (query.minPrice && query.maxPrice) {
      setRange({
        min: parseInt(String(query.minPrice)),
        max: parseInt(String(query.maxPrice)),
      })
    } else {
  ...
  function containsAttrInUrl(type, value) {
    const currentQueries = query[type] ? String(query[type]).split(',') : []
    return currentQueries && currentQueries.includes(value)
  }

  function getUrlForAttrs(type, value) {
    let currentQueries = query[type] ? String(query[type]).split(',') : []
  ...
  function onAttrClick(e, attr, value) {
    if (getUrlForAttrs(attr, value)) {
      ...
      let url = router.pathname.replace('[type]', String(query.type)) + '?'
